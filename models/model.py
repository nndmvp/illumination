...
class DDPM(BaseModel):
    ...
    def feed_data2(self, data):
        self.v_data = self.set_device(data)
    ...
    def optimize_parameters(self):
        ...
        b, c, h, w = self.data['ir'].shape
        l_pix = l_pix.sum() / int(b * c * h * w)
        l_pix.backward()
        self.optG.step()
        # set log
        self.loss_all.append(l_pix.item())

    def test_concat(self, in_channels, img_size_w, img_size_h, continous=False):
        #
        self.netG.eval()
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.sampled_img = self.netG.module.sampling_imgs(
                    in_channels, img_size_w, img_size_h, continous)
            else:
                self.sampled_img = self.netG.sampling_imgs(
                    self.v_data, in_channels, img_size_w, img_size_h, continous)
        self.netG.train()
    ...
    def get_feats(self, t):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                fe, fd = self.netG.module.feats(self.data, t)
            else:
                fe, fd = self.netG.feats(self.data, t)
        self.netG.train()
        return fe, fd
    ...
    def get_current_log(self):
        self.log_dict['l_pix'] = np.average(self.loss_all)
        self.loss_all = []
