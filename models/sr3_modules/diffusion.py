...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, v_data, in_channels, img_size_w, img_size_h, continous=False):
        ...
        if not self.conditional:
            ...
            img = self.q_sample(
                ...
                noise=noise)
            # img = torch.randn((1, in_channels, img_size_w, img_size_h), device=device)
            ret_img = img
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step',
                          total=self.num_timesteps):
                img = self.p_sample(img, torch.full(
                    (1,), i, device=device, dtype=torch.long), condition_x=x_c)
                if i % sample_inter == 0:
    ...
    def sample(self, batch_size=1, continous=False):
        img_size_w = self.image_size_w
        img_size_h = self.image_size_h
        channels = self.channels
        return self.p_sample_loop((batch_size, channels, img_size_w, img_size_h), continous)

    @torch.no_grad()
    def sampling_imgs(self, v_data, in_channels, img_size_w, img_size_h, continous=False):
        return self.p_sample_loop(v_data, in_channels, img_size_w, img_size_h, continous)
